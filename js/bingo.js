var bingo = function(bingoList, size) {
	...
	results.append ("<p>Seed: <strong>" + SEED + "</strong>&emsp;Possible tiles: <strong>" + bingoList[1].length + "</strong>");
	...
	/*$("#bingo tr td:not(.popout), #selected td").toggle(
		function () { $(this).addClass("greensquare"); },
		function () { $(this).removeClass("greensquare"); }
	);*/
	...
	for (var i=1;i<=25;i++) {
		...
		bingoBoard[i] = {difficulty: 1};
		//console.log(bingoBoard[i].difficulty);       //store the difficulty
	}                                          // in order 1-25

	// Starting here I (ping) rip apart the code to remove the balancing stuff.
	// I apologize for everything.
	
	//populate the bingo board in the array
	var gotten = [];
	for (i=1; i<=25; i++) {
		var getDifficulty = 1; // difficulty of current square
		...
		do {
			getDifficulty = 1;
			numtest = (j+RNG)%bingoList[1].length;
			//currentObj = bingoList[getDifficulty][(j+RNG)%bingoList[getDifficulty].length];
			//synergy = checkLine(i, currentObj.types);
			j++;
			found = gotten.indexOf(numtest);
		//} while ((synergy != 0) && (j<bingoList[getDifficulty].length));
		} while (found > -1);

		//bingoBoard[i].types = minSynObj.value.types;
		//bingoBoard[i].name = minSynObj.value[LANG] || minSynObj.value.name;
		//bingoBoard[i].name = currentObj;
		actualBingo[i] = bingoList[1][numtest].name;
		gotten.push(numtest);
